...
from utils.utils import TABLE_NEWS, TABLE_DIGEST, TABLE_PERFORMANCE
...
def create_news_digest_table(conn) -> None:
    sql = f"""
    CREATE TABLE IF NOT EXISTS {TABLE_DIGEST} (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,

        run_id VARCHAR(128) NOT NULL,
        country_code VARCHAR(8) NOT NULL DEFAULT 'HK',
        market_code VARCHAR(16) NOT NULL DEFAULT 'HSI',
        anchor_date DATE DEFAULT NULL,
        lookback_months INT NOT NULL DEFAULT 1,

        model_name VARCHAR(255) NOT NULL,
        prompt_version VARCHAR(64) DEFAULT NULL,

        news_window_start DATE DEFAULT NULL,
        news_window_end DATE DEFAULT NULL,

        used_url_hashes_json JSON DEFAULT NULL,

        analyst_report_json JSON DEFAULT NULL,

        signal_strength VARCHAR(32) DEFAULT NULL,
        ...
        PRIMARY KEY (id),
        UNIQUE KEY uk_news_digestion_run (run_id),

        KEY idx_news_digestion_anchor_date (anchor_date),
        KEY idx_news_digestion_market_code (market_code),
        KEY idx_news_digestion_model_name (model_name),
        KEY idx_news_digestion_signal_strength (signal_strength)
    ) ENGINE=InnoDB
      DEFAULT CHARSET=utf8mb4
      COLLATE=utf8mb4_unicode_ci
    """
    ...
    cursor.execute(sql)
    cursor.close()
    print("Table `news_digestion` checked/created.")
...
def init_database() -> None:
    ...
    try:
        ...
        create_news_digest_table(db_conn)
